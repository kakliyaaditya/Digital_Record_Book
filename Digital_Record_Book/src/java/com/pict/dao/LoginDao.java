...
public class LoginDao {
    ...
    public void showClass(String username, String classes[]) throws ClassNotFoundException, SQLException {
        ...
        conn = ConnectDatabase.getConnection();
        String query1 = "select SubjectName from SubjectTable where SubjectCode=(select SubjectCode from UserInfo where UserID ='" + username.trim() + "' and GroupCode=(select GroupCode from GroupTable where EndRollNo-StartRollNo>70));";
    ...
    public int showClass(String username) throws ClassNotFoundException, SQLException {
        ...
        conn = ConnectDatabase.getConnection();
        String query1 = "select count(SubjectName) from SubjectTable where SubjectCode=(select SubjectCode from UserInfo where UserID ='" + username.trim() + "' and GroupCode=(select GroupCode from GroupTable where EndRollNo-StartRollNo>70));";
    ...
    conn=ConnectDatabase.getConnection();
    String query="update LoginTable set Password='"+newpass.trim()+"' where UserID='"+username.trim()+"' and Password='"+oldpass.trim()+"';";
    ...
    public ResultSet tests(String subjectname) throws SQLException{
        ...
        String query="select TestCode from UnitTest where SubjectCode=(select SubjectCode from SubjectTable where SubjectName='"+subjectname.trim()+"');";
        Statement stmt=conn.createStatement();
